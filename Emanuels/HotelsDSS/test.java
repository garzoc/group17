...
public class test {
	...
	try{
		...
		TextField searchField = (TextField) Main.scene.lookup("#searchField");
		//TextField searchText = (TextField) Main.scene3.lookup("#searchField");
        
		//searchText =
		String sql = "SELECT * FROM Hotel WHERE Name LIKE '" + searchField.getText().toString() +"%'";
		//String sqltest = "SELECT * FROM Hotel WHERE Name LIKE '" + searchText.getText().toString() +"%'";
		//Resultset for outputting the hotels
		System.out.println(searchField.getText());
		ResultSet searchName = statement.executeQuery(sql);
		//ResultSet searchNameTest = statement.executeQuery(sqltest);
		
		
		int tupleCount = 0;
        
        //Add text to the labels
		while (searchName.next()){ //.next();
			//ObservableList<String> row = FXCollections.observableArrayList();
			hotelLabel[tupleCount].setText("Name: " + searchName.getString("Name"));
			//observableList.add(viewing.getString("Name"));
			addressLabel[tupleCount].setText("Address: " + searchName.getString("Address"));
			//observableList.add(viewing.getString("Address"));
			priceLabel[tupleCount].setText("Price: "+searchName.getString("Price") + " SEK/Night");
			//observableList.add(viewing.getString("Price"));
	        starsLabel[tupleCount].setText("Stars: "+searchName.getString("Stars"));
	        //observableList.add(viewing.getString("Stars"));
	        
	        //Add the images
	        File file = new File(searchName.getString("Img"));
...
//	        
	...
	public static void searchTestData(){
...
//           	smallGrid.add(starsLabel, numCol+1, i+3);
      	 	...
      	 	numCol++;
       			...
       			sc = (ScrollPane) Main.scene4.lookup("#scroll");
       			sc.setContent(grid);
